...
class LineRangeListTest {
    ...
    void shouldStoreRangeWithOneLines() {
        LineRangeList list = new LineRangeList();
        LineRange range = new LineRangeBuilder().setLineRange(0,0).build();
        list.add(range);
        assertThat(list).containsExactly(range);
    }
    ...
    void shouldProvideContains() {
        ...
        assertThat(last).containsExactly(new LineRangeBuilder().setLineRange(0,1).build(),
        ...
        assertThat(middle).containsExactly(new LineRangeBuilder().setLineRange(0,1).build(),
                new LineRangeBuilder().setLineRange(4,5).build());

        LineRangeList first = createThreeElements();
        assertThat(first.contains(new LineRangeBuilder().setLineRange(0,1).build())).isTrue();
        assertThat(first.contains(new LineRangeBuilder().setLineRange(2,3).build())).isTrue();
        assertThat(first.contains(new LineRangeBuilder().setLineRange(4,5).build())).isTrue();

        first.remove(new LineRangeBuilder().setLineRange(0,1).build());
        assertThat(first).containsExactly(new LineRangeBuilder().setLineRange(2,3).build(),
                new LineRangeBuilder().setLineRange(4,5).build());
