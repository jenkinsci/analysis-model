...
class PVSStudioParserTest extends AbstractParserTest {
    ...
    protected void assertThatIssuesArePresent(final Report report, final SoftAssertions softly) {
        ...
        softly.assertThat(report.stream().map(Issue::getSeverity).filter(s -> s == Severity.WARNING_NORMAL).count()).isEqualTo(24);
        softly.assertThat(report.stream().map(Issue::getSeverity).filter(s -> s == Severity.WARNING_LOW).count()).isEqualTo(3);

        softly.assertThat(report.stream().map(Issue::getType).filter("GENERAL Analysis"::equals).count()).isEqualTo(7);
        softly.assertThat(report.stream().map(Issue::getType).filter("Micro-OPTIMIZATION"::equals).count()).isEqualTo(1);
        ...
        softly.assertThat(report.stream().map(Issue::getType).filter("Unknown"::equals).count()).isEqualTo(2);
