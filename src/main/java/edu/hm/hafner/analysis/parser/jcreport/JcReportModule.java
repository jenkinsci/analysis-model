...
/**
 * This class is the 'module' that decides WHAT the Digester has to parse and to WHICH property the value given.
 *
...
public class JcReportModule extends AbstractRulesModule {
    ...
    protected void configure() {
         ...
         */
        forPattern("report/file").createObject().ofType(File.class.getCanonicalName()).then().setNext("addFile").then()
                                 .setProperties().then().setProperties().addAlias("package").forProperty("packageName")
        ...
        forPattern("report/file/item").createObject().ofType(Item.class.getCanonicalName()).then().setNext("addItem")
                                      .then().setProperties().addAlias("finding-type").forProperty("findingtype").then()
                                      .setProperties().addAlias("end-line").forProperty("endline").then()
                                      .setProperties().addAlias("end-column").forProperty("endcolumn");
