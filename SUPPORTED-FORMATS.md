<!--- DO NOT EDIT - Generated by ParserRegistry at 2021-11-24T08:25:06.342217-->
    ...
    <tbody>
        ...
        <td colspan="4">
            :bulb: run 
                <pre><code>
steps{
    sh 'CodeChecker analyze compilecommands.json --output dir_to_write_plist_files_into'
    sh 'CodeChecker parse dir_to_write_plist_files_into > report.txt'
    recordIssues tool:analysisParser(id: 'code-checker', pattern: 'report.txt')
}
</code></pre>
        </td>
        <tr>
            <td>
                code-checker-clang-static-analysis
            </td>
            <td>
                -
            </td>
            <td>
                CodeCheckerClangStaticAnalysis
            </td>
            <td>
                -
            </td>
        ...
        <td colspan="4">
            :bulb: CodeChecker can run clang-tidy and clang static analysis in one run and
...
This name alias enables warnings-ng-plugin to parse the same CodeChecker report.txt as a distinct tool. 
            <p>
                <pre><code>
steps{
    sh 'CodeChecker analyze compilecommands.json --analyzers clangsa --output dir_to_write_plist_files_into'
    sh 'CodeChecker parse dir_to_write_plist_files_into > report.txt'
    recordIssues tool:analysisParser(id: 'code-checker-clang-static-analysis', pattern: 'report.txt')
}
                </code></pre>
            </p>
        </td>
        <tr>
            <td>
                code-checker-clang-tidy
            </td>
            <td>
                -
            </td>
            <td>
                CodeCheckerClangTidy
            </td>
            <td>
                -
            </td>
...
<pre><code>
steps{
    sh 'CodeChecker analyze compilecommands.json --analyzers clang-tidy --output dir_to_write_plist_files_into'
    sh 'CodeChecker parse dir_to_write_plist_files_into > report.txt'
    recordIssues tool:analysisParser(id: 'code-checker-clang-tidy', pattern: 'report.txt')
}
</code></pre>
</td>
        <tr>
        ...
        <tr>
            ...
            <td>
                <img src="https://github.com/facebook/flow/blob/master/website/favicon.svg" alt="Flow" height="64" width="64">
            </td>
            <td>
...
analyze - iccxxxxcompiler_opts cstat2.c</code></pre>For details check the IAR C-STAT guide.</p>
            </td>
        </tr>
        <tr>
            <td>
        ...
        <tr>
            ...
            <td>
                <img src="https://github.com/facebook/infer/blob/master/website/static/img/logo.png" alt="Infer" height="64" width="64">
            </td>
            <td>
